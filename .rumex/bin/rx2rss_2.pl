...
# Einlesen der URL
# und ausschneiden des Dateinamens
my ($url, $file, $rx_end, $xml_end ) = split('\+', $ARGV[0]);
my $file_rx = $file.".".$rx_end;
my $file_xml = $file.".".$xml_end;
...
print "<channel>\n";
print "<atom:link href=\"$url/$file_xml\" rel=\"self\" type=\"application/rss+xml\" />\n";
